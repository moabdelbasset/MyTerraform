resource "null_resource" "AbdelBassetWebserver1HTTPD" {
 depends_on = [oci_core_instance.AbdelBassetWebserver1]
 provisioner "remote-exec" {
        connection {
                type     = "ssh"
                user     = "opc"
                        host     = data.oci_core_vnic.AbdelBassetWebserver1_VNIC1.public_ip_address
